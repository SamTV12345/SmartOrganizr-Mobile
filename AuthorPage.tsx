...
export const AuthorPage = () => {
    ...
    useEffect(() => {
        if (authors === undefined && accessToken && keycloakConfig) {
            const authorLink = keycloakConfig._links.author.href.substring(0, keycloakConfig._links.author.href.lastIndexOf("/")) + "?page=0"
            console.log(authorLink)
            dispatch(setBaseURL(keycloakConfig._links.author.href.substring(0, keycloakConfig._links.author.href.indexOf('/authors'))))
    ...
    const AuthorView:FC<AuthorPageProps> = ({ navigation} )=>{
       return <View>
           {authors&&
               <FlatList data={authors._embedded.authorRepresentationModelList}
